...
for c in category:  ##AA: controllare se è int se funziona il primo controllo
    ...
    subprocess.run(
        ["./plink", "--bfile", args.bfile, "--keep", os.path.join(folder, str(c) + "_patients.txt"), "--make-bed", "--out",
...
for c in category:
    subprocess.run(
        ["./plink", "--bfile", os.path.join(folder,  str(c)), "--extract", os.path.join(folder, "SNPS_to_keep.txt"), "--recodeA", "--out",
...
with open(os.path.join(folder, "merged.raw"), "w") as merge:
    for i in range(0, len(category)):
        ...
        for line in open(os.path.join(folder,  str(category[i])) + '_recoded.raw'):
            ...
            tmp = tmp[6:]
            if iline == 0:
                if i == 0:  # write header only on firt file, snps corrersponds
